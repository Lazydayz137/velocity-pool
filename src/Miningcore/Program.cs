...
public class Program : BackgroundService
{
    ...
    private async Task RunPool(PoolConfig poolConfig, Dictionary<string, CoinTemplate> coinTemplates, CancellationToken ct)
    {
        ...
        Meta<Lazy<IMiningPool, CoinFamilyAttribute>> poolImplMeta;

        // Check if a specific job manager is requested
        if (!string.IsNullOrEmpty(poolConfig.JobManager))
        {
            // Find the job manager by name
            poolImplMeta = container.Resolve<IEnumerable<Meta<Lazy<IMiningPool, CoinFamilyAttribute>>>>()
                .FirstOrDefault(x => x.Value.Value.GetType().Name.Equals(poolConfig.JobManager, StringComparison.OrdinalIgnoreCase));

            if (poolImplMeta == null)
                throw new PoolStartupException($"Unable to find requested job manager '{poolConfig.JobManager}'");
        }
        else
        {
            // Use the default job manager for the coin family
            poolImplMeta = container.Resolve<IEnumerable<Meta<Lazy<IMiningPool, CoinFamilyAttribute>>>>()
                .First(x => x.Value.Metadata.SupportedFamilies.Contains(poolConfig.Template.Family));
        }

        var poolImpl = poolImplMeta.Value;
